...
function onStart(alicPockers, bobPockers, laiziPocker) {
    ...
    // 获取用户最大的一手牌的牌型以及牌值
    // let aUserPockers = handleCardsLength === 5 ? getUserPockersFive(aHandlePockers, laiziPocker) : getUserPockersSenven(aHandlePockers, laiziPocker)
    let bUserPockers = handleCardsLength === 5 ? getUserPockersFive(bHandlePockers, laiziPocker) : getUserPockersSenven(bHandlePockers, laiziPocker)
...
function getUserPockersSenven(pockers, laiziPocker) {
    ...
    for (let index = 0; index < pockers.length; index++) {
        ...
        // 获取用户的牌型
        let userPockers = getUserPockersFive(element, laiziPocker)
...
function compairHandleHightPockers(aPockers, bPockers) {
    const _pairArr = [TYPE.HIGHTCARD, TYPE.FLUSH, TYPE.STRAIGHT_FLUSH]
    ...
    if (_pairMax.indexOf(aPockers.type) > -1) return { result: 0, pockers: aPockers } 

    // 单牌或者普通顺子,确定癞子具体为什么值, 有癞子又不是同花的的时候不可能有癞子
    if (!!aPockers.laizi && _pairArr.indexOf(aPockers.type) > -1) aPockers.codes = ensureLaizi(aPockers, _pairArr)
    if (!!bPockers.laizi && _pairArr.indexOf(bPockers.type) > -1) bPockers.codes = ensureLaizi(bPockers, _pairArr)
...
// 确定具体的癞子值
function ensureLaizi(userPockers, pairTypeArr) {
    // 癞子当A
    let _codes = userPockers.codes, len = userPockers.codes.length, isFlush = pairTypeArr.indexOf(userPockers.type) > -1

    // 如果是单牌且最大的牌面值不是A,癞子当A
    if (!isFlush && +userPockers.codes[userPockers.codes.length - 1] !== CODE['A']) {
        _codes.push(CODE['A']+'')
        return _codes
    }
...
function getCardsMaxNumber(pockersObj, codes, laizi) {
    ...
    if (max > 1) {
        ...
        codes.splice(codes.indexOf(_firstInsert), 1)
        if (!!_secondInsert) {
            codes.splice(codes.indexOf(_secondInsert), 1)
            codes.push(_secondInsert)            
        }
        codes.push(_firstInsert)        
    }
...
function checkFlush(codes, laizi) {
    ...
    // 如果不是最大的顺子，且手牌中有A,手牌可以组成顺子，则为最小的顺子
    let _minDistance = _codes[_codes.length - 2] - _codes[0]
    if (!isMaxFlush && hasA && _codes[0] === '2' && ((!laizi && _minDistance === 3) || (!!laizi && _minDistance <= 3))) {
        isFlush = true
        _codes.pop()
        _codes.unshift('1')
    }
